...
\begin{table}[t]
\centering
\caption{Pre-registered anchor events, fixed before any window count was computed. Per-platform counts in the 48-hour and two-week windows are \blocked{the archive and the density audit}; the provisional inclusion thresholds are 200 comments per platform-event cell in the two-week window and 50 in the 48-hour window \todo{confirm thresholds before counting}. Dates to be verified against official records before the audit runs.}
\label{tab:anchors}
\begin{tabular}{llr}
\toprule
Event & Date & Counts \\
