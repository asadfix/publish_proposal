...
\begin{enumerate}
  ...
  \item We specify a six-step protocol: same-event divergence testing;
    ...
    \citep{robitzsch2023}.
  \item We quantify the cost of assuming equivalence in a simulation with known ground truth: platforms that read the same latent opinion through different measurement functions produce a pooled index that distorts event shifts and can report the wrong sign of aggregate sentiment, while a measurement-adjusted index recovers the truth. \todo{Insert the final numbers once the generating parameters are pre-registered.}
  \item We ground the protocol in a case study of the dissertation-stage instrument, a fine-tuned BERT classifier over 255,446 UK economic-policy comments from Twitter/X, Reddit, YouTube, and Quora, whose execution records supply the paper's motivating evidence; the full four-platform application, under fresh collection with the pre-registered design, is designated as the follow-up study.
\end{enumerate}
